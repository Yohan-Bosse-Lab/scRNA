...
cnv_plots = list()
for(i in 1:2){
  combined_permethod[[1]]@meta.data$cnv_score[combined_permethod[[1]]@meta.data$cnv_score>0.03] =0.012
   combined_permethod[[2]]@meta.data$cnv_score[combined_permethod[[2]]@meta.data$cnv_score>0.027] =0.027
  combined_permethod_percelltype[[1]][[i]]@meta.data$cnv_score[combined_permethod_percelltype[[1]][[i]]@meta.data$cnv_score>0.009]=0.01
  
cnv_plots[[i]] = FeaturePlot(combined_permethod[[i]],order = T,reduction = 'umap',features = 'cnv_score',pt.size = 0.2,cols = c('#D3D3D3ff','#ff0000ff')) + ggtitle('') + guides(shape='none') + theme(legend.position = "right",axis.title = element_text(size = 8),axis.text =element_text(size = 8) )

cnv_plots[[i]]$labels$colour = 'CNV score'  
...
cnv_plots[[i+2]] = FeaturePlot(combined_permethod_percelltype[[1]][[i]],reduction = 'umap',features = 'cnv_score',order = T, pt.size = 0.5,cols = c('#D3D3D3ff','#ff0000ff'),min.cutoff = 0,max.cutoff = 0.016) + guides(shape='none') + ggtitle('') + theme(legend.position = "right",axis.title = element_text(size = 8),axis.text =element_text(size = 8) )

cnv_plots[[i+2]]$labels$colour = 'CNV level'  
...
#cell types plots  FINE LEVELS
  ...
  pdf(file.path(params$datapath,'../results/Figures_August2023/Fig4_UMAPs_CTA_tumor24.pdf'),width = 12,height = 6) 
 (plot_SC + barplots_general[[1]] + plot_SN + barplots_general[[2]] + plot_layout(widths = c(10,1,10,1))) / 
 ((cnv_plots[[1]] | empty_placeholder | cnv_plots[[2]] | empty_placeholder) + plot_layout(widths = c(10,1,10,1))) + plot_annotation(tag_levels = list(c('A','','B','','C','','D','')))
dev.off()
```
...
cnvMean = ggplot(metadata_simplified, aes(cnv_score,predicted.ann_finest_level)) + geom_boxplot(fill = "orange") + scale_x_continuous(trans= 'log10',breaks = c(0.0001,0.001,0.01), label = c(0.0001,0.001,0.01),limits = c(0.00004,0.02)) +
  ...
  sc_sn_plots[[i]] = fraction_per_patient|number_of_cells|number_of_UMI|cnvMean
}

pdf(file.path(params$datapath,'../results/Figures_August2023/Fig3_barplots_lambrechtsT24.pdf'),width = 20,height = 12) 
