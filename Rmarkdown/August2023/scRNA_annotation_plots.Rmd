...
#pdf(file.path(params$datapath,'../results/Figures_July2023/Figure3_annotation_scores24.pdf'),width = 8,height = 7)
#plot_annot_fractions #/ plot_annot_score +  plot_annotation(tag_levels = 'A')
#dev.off()
...
#how many cell types
dim(metadata %>% 
            group_by(predicted.ann_level_3) %>%
...
data_summarised$method[data_summarised$method == 'immune depleted cells'] = 'Immune\ndepleted\ncells'

data_summarised$method = factor(data_summarised$method, levels = c('Cell','Immune\ndepleted\ncells','Nucleus'))
...
bargplots[[1]] = ggplot(data=data_summarised, aes(x=method, y=n, fill=predicted.ann_level_1)) +
  ...
  ggtitle('Cell Types ~ level1') + ylab("Number of Cells")
...
for(i in seq_along((celltypes)))
  ...
  colors = rgb.palette(length(unique(data$predicted.ann_level_3))-1)
  colors[length(colors)+1] = 'orange'

  p = ggplot(data=data, aes(x=method, y=n, fill=predicted.ann_level_3)) +
    geom_bar(stat="identity") +
    scale_fill_manual('Cell Types Annotations',values=colors)+ 
    facet_wrap(~type,nrow = 1, ncol = 2) +
    ggtitle(paste0(celltypes[i],' ~ level3')) + ylab("")
...
```{r lineplots}
#just make sure you have all the categories...
...
tumor_normal_transition = ggplot(data_summarised2, aes(x=predicted.ann_level_3,y=fractions,fill=method)) +  
    ...
    facet_grid( . ~predicted.ann_level_1, scales = "free_x", space = "free_x",switch = 'y') + scale_y_continuous(trans='log10',breaks = c(0.01,0.1,1,10,100), label = c(0.01,0.1,1,10,100)) +
...
#clean-up a bit
#data_summarised_fraction = data_summarised_fraction[!(data_summarised_fraction$predicted.ann_level_3 %in% c('SM activated stress response','None','Submucosal Secretory','Basal')),]
...
lineplots <- ggplot(data_summarised_fraction, aes(x=type, y=fraction, group=method)) +
  ...
  scale_color_manual(values = hue_pal()(3)[c(1,3)])+
...
pdf(file.path(params$datapath,paste0('../results/Figures_Oct2023/Figure8_cells24.pdf')),width = 12,height = 8)
 ((tumor_normal_transition + lineplots)/guide_area()) + plot_layout(height = c(10,1),guides = 'collect') + plot_annotation(tag_levels = 'A')
dev.off()
```
...
pdf(file.path(params$datapath,paste0('../results/Figures_Oct2023/Figure2.pdf')),width = 11,height = 6)
...
metadata_subset = metadata[metadata$method!='immune depleted cells',]
metadata_subset = metadata_subset[metadata_subset$type!='Normal',]
metadata_subset$cnv_score_log = log(metadata_subset$cnv_score,10)
#metadata_subset = metadata_subset[sample(1:20000,1000),]

cnv_annot_corplot = ggplot(data = metadata_subset, aes(y= cnv_score,x = predicted.ann_level_3.score)) +
...
pdf(file.path(params$datapath,paste0('../results/Figures_Oct2023/FigS3.pdf')),width = 8,height = 4)
cnv_annot_corplot
dev.off()
```
...
pdf(file.path(params$datapath,'../results/Figures_Oct2023/FigS2.pdf'),width = 6,height = 4) 
sil_plot
dev.off()
```
...
table = table[-62,]
#table = table[rowSums(table[,7:9])>100,]

write.csv(table,file.path(params$datapath,'../results/Figures_Oct2023/annotation_table_finest.csv'),row.names = F)
```
...
###can we add nones / unknowns in this loop? 
#
for(i in seq_along(hlca$children))
  {
...
write.table(print(hlca,'Cell_Normal','Nucleus_Normal','depleted_Normal','Cell_Tumor','Nucleus_Tumor','depleted_Tumor','annotlevel','finest',limit =500),file.path(params$datapath,'../results/Figures_Oct2023/hierarchy_annotation.csv'),row.names = F,quote=F,sep=',')
