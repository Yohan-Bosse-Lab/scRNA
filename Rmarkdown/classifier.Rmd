...
if(params$v2 == TRUE) {
  markers_genes_azimuth = read.csv('C:/Users/renseb01/Documents/scRNA/scRNA/data/HLCA/Marker_genes_Azimuth.csv',row.names = 1,check.names = F)

  markers_genes =  data.frame(t(markers_genes_azimuth))
  colnames(markers_genes) = rownames(markers_genes_azimuth)

  for(m in seq_along(markers_genes)){
    split_genes = strsplit(markers_genes[,m],split = ', ')[[1]]
    markers_genes[1:length(split_genes),m] = split_genes
  }
} else {
...
test = sample(c(1:584884)[-train],40000)
...
table(test_set@meta.data$ann_finest_level_recoded)
```
...
for(i in seq_along(markers_genes))
  ...
  if(i%%1==0) print(paste0(Sys.time()," --- ",i, ' of ',number))
  
  model_filepath = paste0('C:/Users/renseb01/Documents/scRNA/scRNA/data/HLCA/models_azimuth/')
...
datatable(finest,caption = htmltools::tags$caption(htmltools::strong("Finest Level Predictions"), style="color:darkred"))

datatable(level1,caption = htmltools::tags$caption(htmltools::strong("Level 1 Predictions"), style="color:darkred"))
