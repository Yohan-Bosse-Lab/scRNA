...
QC_filters=data.frame(nCount_min=rep(0,4),nCount_max=rep(0,4),
                    nFeature_RNA_min=rep(0,4),nFeature_RNA_min=rep(0,4),
                    percent.mt_min=rep(0,4),percent.mt_max=rep(0,4))
...
#for(i in 1:4) {
  ...
  if(class(temp)!='try-error') {seurat.objects[[i]]=temp}
...
```{r celltypes,echo = F} 
all4 = table(seurat.objects[[1]]@meta.data$most_probable_cell_type)
sn4 = table(seurat.objects[[2]]@meta.data$most_probable_cell_type)
all6 = table(seurat.objects[[3]]@meta.data$most_probable_cell_type)
sn6 = table(seurat.objects[[4]]@meta.data$most_probable_cell_type)

all4_fractions = signif(all4/sum(all4)*100,4)
sn4_fractions = signif(sn4/sum(sn4)*100,4)
all6_fractions = signif(all6/sum(all6)*100,4)
sn6_fractions = signif(sn6/sum(sn6)*100,4)

sn = merge(as.data.frame(sn4_fractions),as.data.frame(sn6_fractions),by= 'Var1',all =T)
all= merge(as.data.frame(all4_fractions),as.data.frame(all6_fractions),by= 'Var1',all =T)
...
sn_all[nrow(sn_all)+1,] = list('Total number of cells',sum(sn4),sum(sn6),sum(all4),sum(all6))

datatable(sn_all,caption = htmltools::tags$caption(htmltools::strong("Predicted Cell Types (fraction of total)"), style="color:darkred"))
```
...
p2 <- DimPlot(adeno.combined.sct, reduction = "umap", group.by = "method", cols = 'Set1')
p1
p2
```
...
p3 = DimPlot(adeno.combined.sct, reduction = "umap",label = T,group.by =  'integrated_snn_res.0.5') + theme(legend.position = "none")
...
ct_summarised <- adeno.combined.sct@meta.data %>%
  group_by(most_probable_cell_type,integrated_snn_res.0.5,.drop=F) %>% 
...
plot_SN = ggplot(data=ct_summarised_SN, aes(y=n, x=integrated_snn_res.0.5, fill = most_probable_cell_type)) + 
...
plot_ALL = ggplot(data=ct_summarised_ALL, aes(y=n, x=integrated_snn_res.0.5, fill = most_probable_cell_type)) + 
    geom_bar(stat="identity") + 
...
ct_summarised_DT = xtabs(n ~ ., ct_summarised,sparse = F)
ct_summarised_DT_matrix = matrix(as.vector(ct_summarised_DT),nrow = nrow(ct_summarised_DT), ncol = ncol(ct_summarised_DT))
colnames(ct_summarised_DT_matrix) = paste0('cluster',colnames(ct_summarised_DT));rownames(ct_summarised_DT_matrix) = rownames(ct_summarised_DT)

datatable(ct_summarised_DT_matrix,caption = htmltools::tags$caption(htmltools::strong("Predicted Cell Type Annotations per cluster"), style="color:darkred"))
```
