...
#for(i in 1:1) {
  ...
  seurat.objects[[i]]@meta.data$method = ifelse(temp[2]=='SN',temp[2],'SC')
...
pK_empirical = c(0.25,0.03,0.2,0.01)
...
#  sn_all[nrow(sn_all),-1] = lapply(cell_type_summary,sum)
  ...
  if(level == 'most_probable_cell_type') {
    final_summary[[1]] = sn_all;
    
    
    summary_statistics = t(data.frame(`Total number of cells` = sapply(cell_type_summary,sum),
               `Diversity (Shannon) Index` = signif(sapply(cell_type_summary,diversity),4),
               `Number of unique cell types` = sapply(cell_type_summary,length),
               check.names = F))
    
    colnames(summary_statistics) = c('Single cells - P4','Single nuclei - P4','Single cells - P6','Single nuclei - P6')
    
    summary_statistics = summary_statistics[,c(2,4,1,3)]
    
    final_summary[[3]] = summary_statistics
  }
...
datatable(final_summary[[3]],caption = htmltools::tags$caption(htmltools::strong("Summary Statistics"), style="color:darkred"))
...
ct_summarised_SN <- adeno.combined.sct@meta.data %>%
  ...
  summarise(n = length(integrated_snn_res.0.5),.groups='keep') %>% 
  filter(integrated_snn_res.0.5 == 5|integrated_snn_res.0.5 == 6|integrated_snn_res.0.5 == 7|integrated_snn_res.0.5 == 8|integrated_snn_res.0.5 == 18) %>%
  filter(n>100) %>% filter(most_probable_cell_type!='unknown')

ct_summarised_ALL <- adeno.combined.sct@meta.data %>%
  group_by(most_probable_cell_type,integrated_snn_res.0.5,.drop=) %>% 
  summarise(n = length(integrated_snn_res.0.5),.groups='keep') %>% 
  filter(integrated_snn_res.0.5 == 2|integrated_snn_res.0.5 == 11|integrated_snn_res.0.5 == 14|integrated_snn_res.0.5 == 15|integrated_snn_res.0.5 == 16) %>%
  filter(n>100) %>% filter(most_probable_cell_type!='unknown')
