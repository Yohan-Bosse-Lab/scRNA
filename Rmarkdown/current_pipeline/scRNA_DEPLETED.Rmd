...
#gene variable DF
plot_depleted_all2 = ggplot(data,aes(x= Dim.1,y=Dim.2, fill = `method X Patients` )) + 
...
#prepare output df
stress_genes = data.frame(gene = rownames(seurat.objects[[i]]@assays$RNA)[rownames(seurat.objects[[i]]@assays$RNA) %in% stress_references$gene_symbol])
nonstressgenes = rownames(combined@assays$RNA)[rownames(combined@assays$RNA) %in% stress_genes[,1]==FALSE]

# 
combined = subset(combined_raw, subset = predicted.ann_level_1 != 'Unclassified')
combined = subset(combined, subset = type != 'Tumor')
combined = subset(combined,features = nonstressgenes)
combined@meta.data$method = gsub('immune','Immune',combined@meta.data$method)
combined@meta.data$met_type = paste0(combined@meta.data$method, '_',combined@meta.data$type)
...
p_tissuemethod = ggplot(data,aes(x= Dim.1,y=Dim.2, fill = `method X Patients`)) +
    geom_hline(yintercept=0,linetype="dashed") + 
    geom_vline(xintercept=0,linetype="dashed") + 
  ...
  #  ggrepel::geom_text_repel(aes(label = Patients)) +
    ...
    theme(legend.position = 'right') +
    scale_fill_manual(values = c(RColorBrewer::brewer.pal(n = 11, name = "RdYlBu")[c(4:1,8:11)],RColorBrewer::brewer.pal(n = 9, name = "Greens")[c(3,5,7,9)])  ) +
    ggtitle(paste0(gsub('_','',ct[i]),' cells ~ gene expression'))
