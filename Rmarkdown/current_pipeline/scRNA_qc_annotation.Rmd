...
# ADD the cnv data and save it again. 
```{r annotation_cnv}
...
#for(i in c(3,23)){
  ...
  #infercnv
  infercnv_out = suppressMessages(infercnv_wrapper(seurat_obj = seurat.objects[[i]],
                              quantile = 0.5, 
                              out.dir = file.path(params$datapath,'infercnv/24samples/',filenames[i]),
                              level = 'predicted.ann_level_1',
                              gene_order_file = file.path(params$datapath,'gencode.v43.primary_assembly.annotation.positional')))
  
  seurat.objects[[i]] = infercnv_out[[1]]
  
  print(infercnv_out[[2]])
  print(paste0(i,' --- Time: ',Sys.time(),' -- seurat object ready (ANNOTATION, CNV)'))
}
