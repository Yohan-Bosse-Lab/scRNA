...
#bulk_final_summary_final = bulk_final_summary[,-c(1,8)]
...
#pdf(file.path(params$datapath,'../results/Figures_March2024/Fig_gexpr_cor.pdf'),width = 15,height = 8) 
...
#corrRect(p, name = c("Normal_Cell_Immune","Tumor_Nucleus_Epithelial",'Normal_Nucleus_Stroma'))
#dev.off()
...
gene_variables$`gene loadings` = 'Endothelial'
gene_variables$`gene loadings`[gene_variables$genes %in% Epithelial] = 'Epithelial'
gene_variables$`gene loadings`[gene_variables$genes %in% Stroma] = 'Stroma'
gene_variables$`gene loadings`[gene_variables$genes %in% Immune] = 'Immune'
...
data$`gene loadings` = sapply(strsplit(rownames(data), "_",fixed = T), "[",4)
...
p_level1 = ggplot() + 
    geom_segment(data =gene_variables,  aes(x = rep(0,38), y = rep(0,38), xend = coord.Dim.1*max(abs(data$Dim.1)), yend = coord.Dim.2*max(abs(data$Dim.2)), color = `gene loadings`), arrow = arrow(length = unit(0.2, "cm")), show.legend = T ) + 
  geom_hline(yintercept=0,linetype="dashed") + geom_vline(xintercept=0, linetype="dashed") +
  geom_point(data=data,aes(x= Dim.1,y=Dim.2, fill = `gene loadings`, shape = patients), size = 4,color = 'black', shape = 21,alpha =0.75) +
