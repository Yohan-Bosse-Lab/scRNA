...
```{r UMAPplots}
if(file.exists(file.path(params$datapath,'combined_permethod_pertypeN.rds')) == F){
npc=30
type = c('Normal')
combined_permethod = list()
for(i in 1:2){
  if((i == 1) & (type == 'Normal')) samples = c(1:length(filenames))[regexpr('CST-...._all_',filenames)>0]
  if((i == 2) & (type == 'Normal')) samples = c(1:length(filenames))[regexpr('CST-...._SN_',filenames)>0]
...
saveRDS(combined_permethod,file.path(params$datapath,"combined_permethod_pertypeN.rds"))
} else combined_permethod = readRDS(file.path(params$datapath,'combined_permethod_pertypeN.rds'))

```

# 'SingleCells','SingleNuclei' NORMAL subset epit & Immune
```{r per method, cell cell type normal tissue}
if(file.exists(file.path(params$datapath,'combined_permethod_percelltype.rds')) == F){
npc=30
type = c('Normal')
...
saveRDS(combined_permethod_percelltype,file.path(params$datapath,"combined_permethod_percelltypeN.rds"))
} else combined_permethod_percelltype = readRDS(file.path(params$datapath,'combined_permethod_percelltypeN.rds'))
```
...
#cell types plots  FINE LEVELS
  ...
  if(i == 2 & c == 1) {data_summarised = data_summarised %>% bind_rows(data.frame(predicted.ann_finest_level= c("others"),patient = 'P4',n =0))}
...
#cell types plots  FINE LEVELS
  ...
  pdf(file.path(params$datapath,'../results/UMAPs_CTA.pdf'),width = 12,height = 7) 
 (plot_SC + barplots_general[[1]] + plot_SN + barplots_general[[2]] + plot_layout(widths = c(10,1,10,1))) / 
 (plot_UMAPS[[2]] + barplots[[2]] + plot_UMAPS[[4]] + (barplots[[4]]+guides(fill=guide_legend(ncol=2))) + plot_layout(widths = c(10,1,10,1)))  / (plot_UMAPS[[1]] + barplots[[1]] + plot_UMAPS[[3]] + barplots[[3]] + plot_layout(widths = c(10,1,10,1))) + plot_annotation(tag_levels = list(c('A','','','','B','','','','C','','','')))
  dev.off()
```
# session info  
```{r session, message= T}
...
```{r toto, message= T}
s = 7
seurat.objects[[s]] = RunPCA(seurat.objects[[s]])
seurat.objects[[s]] = RunUMAP(seurat.objects[[s]],dims = 1:30)
seurat.objects[[s]]@meta.data$others = ''
seurat.objects[[s]]@meta.data$others[seurat.objects[[s]]@meta.data$predicted.ann_finest_level!='AT1']='others'
seurat.objects[[s]]@meta.data$others[seurat.objects[[s]]@meta.data$predicted.ann_finest_level=='AT1']='AT1'
ggplotly(DimPlot(seurat.objects[[s]],reduction='umap',group.by= 'others'))


  features <- SelectIntegrationFeatures(object.list = seurat.objects[samples], nfeatures = 2000)
  seurat.objects.method <- PrepSCTIntegration(object.list = seurat.objects[samples], anchor.features = features)
  adeno.anchors <- FindIntegrationAnchors(object.list = seurat.objects.method, normalization.method = "SCT", anchor.features = features)
  ccc <- IntegrateData(anchorset = adeno.anchors, normalization.method = "SCT")
  ccc <- RunPCA(ccc, verbose = FALSE)
  ccc <- RunUMAP(ccc, reduction = "pca", dims = 1:npc, verbose = FALSE)

  
  
  
 a=  DimPlot(combined_permethod_percelltype[[1]][[1]],group.by = "predicted.ann_finest_level",reduction = 'umap',pt.size = 3,cols = 'Set1') + ggtitle('Epithelial -- Cells - Normal tissue')
  b=DimPlot(combined_permethod_percelltypeT[[1]][[1]],group.by = "predicted.ann_finest_level",reduction = 'umap',pt.size = 3,cols = 'Set1')+ ggtitle('Epithelial -- Cells - Tumor tissue')
  c=FeaturePlot(combined_permethod_percelltype[[1]][[1]],features = c('CEACAM6'))+ ggtitle('CEACAM6 -- Cells - Normal tissue')
  d= FeaturePlot(combined_permethod_percelltypeT[[1]][[1]],features = c('CEACAM6'))+ ggtitle('CEACAM6 -- Cells - tumor tissue')
  
  
  pdf(file.path(params$datapath,'../results/CEACAM6.pdf'),width = 12,height = 7) 
  a+b+c+d
  dev.off()
...
pdf(file.path(params$datapath,'../results/barplots_lambrechtsN.pdf'),width = 12,height = 10) 
